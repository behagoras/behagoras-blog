...
const skills: Record<string, string[]> = {
  'Core Web': ['TypeScript', 'JavaScript (ES6+)', 'React', 'Redux', 'HTML5', 'CSS3', 'CSS-in-JS', 'HTTP', 'State Management', 'DOM'],
  'AI-Augmented Dev': ['Claude Code', 'OpenAI Codex', 'Open Claw Automations', 'AI-Assisted Refactoring & Testing', 'Context Engineering', 'Multi-Agent Orchestration'],
  Performance: ['Core Web Vitals', 'Lighthouse CI', 'Code Splitting', 'Lazy Loading', 'PWA', 'Bundle Optimization', 'Caching Strategies'],
  'Build & Testing': ['Webpack', 'Jest', 'Cypress (E2E)', 'Vitest', 'Storybook', 'CI/CD', 'Jenkins', 'Azure DevOps'],
  'Backend & Cloud': ['Node.js', 'API Design', 'REST APIs', 'GraphQL (AppSync)', 'AWS Lambda', 'API Gateway', 'EventBridge', 'S3', 'Serverless Framework'],
  Architecture: ['Microfrontends (Module Federation)', 'Developer Experience (DX)', 'Responsive Design', 'Scalable Systems', 'Event-Driven Patterns'],
  Practices: ['Git', 'Agile/Scrum', 'OOP', 'SOLID Principles', 'TDD', 'Code Reviews', 'Technical Mentorship'],
}

type TimelineEntry = {
  id: string; year: string; endYear: string; company: string; title: string; location: string; bullets: string[]; type: 'role' | 'education'
}

const timeline: TimelineEntry[] = [
  {
    id: 'autodesk', year: '2019', endYear: 'Present', company: 'Autodesk',
    title: 'Full Stack Engineer | Team Lead | Platform', location: 'Mexico City, MX', type: 'role',
    bullets: [
      ...
      'Adopted Claude Code as a daily engineering tool for AI-augmented architecture design, code generation, refactoring, and testing, using multi-agent orchestration across the platform.',
    ],
  },
  {
    id: 'social-piper', year: '2019', endYear: '2019', company: 'Social Piper',
    title: 'Senior Front-End Developer', location: 'Mexico City, MX', type: 'role',
    bullets: [
  ...
  {
    id: 'qubit-works', year: '2015', endYear: '2018', company: 'Qubit Works',
    title: 'Head of Operations / Lead Engineer', location: 'Mexico City, MX', type: 'role',
    bullets: [
  ...
  {
    id: 'farmaldea', year: '2014', endYear: '2016', company: 'Farmaldea',
    title: 'Head of Technology', location: 'Mexico City, MX', type: 'role',
    bullets: [
  ...
  {
    id: 'ibero', year: '2014', endYear: '', company: 'Universidad Iberoamericana',
    title: 'B.A. | Interactive Design', location: 'Mexico City, MX', type: 'education',
    bullets: ['Speciality in Software Development, Digital Systems, and UX/UI. Foundation in user-centered design and interaction patterns.'],
  },
  {
    id: 'platzi', year: '2021', endYear: '', company: 'Platzi Master Bootcamp',
    title: 'Selective Program (Top 1% LATAM)', location: '', type: 'education',
    bullets: ['Completed 100+ advanced courses in computer science, React, Node.js, AWS, algorithms, and distributed systems. Selected as one of the top 1% of the best students of the year 2019.'],
  },
]
...
export default function CVPage() {
  ...
  useEffect(() => {
    ...
    const observeAll = (ref: React.RefObject<HTMLDivElement | null>) => {
      ref.current?.querySelectorAll('.' + styles.timelineItem)?.forEach((node) => observer.observe(node))
    }
  ...
  return (
    <Layout>
      <NextSeo title="CV" description="David Behar Lombrozo - Software Developer CV. 10+ years building scalable web applications with React, TypeScript, Node.js, and AWS." canonical="https://behar.io/cv" />
            ...
            {Object.entries(skills).map(([group, tags]) => (
              <div key={group} className={styles.skillGroup}>
                ...
                <div className={styles.skillPills}>
                  {tags.map((tag) => (<span key={tag} className={styles.pill}>{tag}</span>))}
                </div>
              </div>
            ))}
          </div>
        ...
        <section className={styles.timelineSection} ref={timelineRef}>
          ...
          <div className={styles.timeline}>
            ...
            {timeline.filter(e => e.type === 'role').map((entry, i) => (
              <article key={entry.id} className={styles.timelineItem + ' ' + (i % 2 === 0 ? styles.left : styles.right)}>
                <div className={styles.timelineDot} aria-hidden="true" />
                <div className={styles.yearMarker}>{entry.year === entry.endYear ? entry.year : entry.year + ' - ' + entry.endYear}</div>
                <div className={styles.card}>
                  ...
                  <ul className={styles.bullets}>{entry.bullets.map((b, j) => (<li key={j}>{b}</li>))}</ul>
                </div>
              </article>
            ))}
          </div>
