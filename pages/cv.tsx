...
const ExperienceEntry = ({
  hash, company, role, location, date, bullets,
}: {
  hash: string; company: string; role: string; location: string; date: string; bullets: string[]
}) => (
  <div className="mb-8">
    <div style={{ color: '#e3b341' }} className="break-all">commit {hash}</div>
    <div style={{ color: '#c9d1d9' }}>
      Company: <span className="font-bold" style={{ color: '#56d4dd' }}>{company}</span>
      <span style={{ color: '#6e7681' }}>{' // '}{location}</span>
    </div>
    <div style={{ color: '#c9d1d9' }}>Role:{'    '}<span style={{ color: '#7ee787' }}>{role}</span></div>
    <div style={{ color: '#c9d1d9' }}>Date:{'    '}<span style={{ color: '#6e7681' }}>{date}</span></div>
...
export default function CVPage() {
  return (
    <>
      ...
      <div className="min-h-screen p-2 sm:p-4 md:p-8" style={{ fontFamily: 'ui-monospace, SFMono-Regular, "SF Mono", Menlo, Consolas, "Liberation Mono", monospace', fontSize: '13px', lineHeight: '1.7', background: '#0d1117', color: '#c9d1d9' }}>
        <div className="max-w-4xl mx-auto mb-4">
          <Link href="/" className="text-xs transition-colors" style={{ color: '#6e7681' }}>{'<'}- back to behar.io</Link>
        </div>
        ...
        <div className="max-w-4xl mx-auto rounded-lg overflow-hidden shadow-2xl" style={{ border: '1px solid #30363d' }}>
          ...
          <div className="flex items-center px-4 py-2.5 gap-2" style={{ background: '#161b22' }}>
            ...
            <div className="flex-1 text-center text-xs" style={{ color: '#6e7681' }}>david-behar-cv -- bash -- 80x42</div>
          </div>

          {/* Terminal body */}
          <div className="p-4 sm:p-6 md:p-8 overflow-x-hidden" style={{ background: '#0d1117' }}>
            <div className="text-xs mb-6" style={{ color: '#484f58' }}>Last login: Mon Apr  7 09:00:00 2026 on ttys000</div>
 ...
 |____/ \\__,_| \\_/ |_|\\__,_| |____/ \\___|_| |_|\\__,_|_|`}</pre>
            ...
            <div className="mt-3 mb-1">
              <div className="text-xs mb-3" style={{ color: '#484f58' }}>{'// installed packages (skills v10.0.0)'}</div>
        ...
        <div className="max-w-4xl mx-auto mt-6 text-center text-xs" style={{ color: '#484f58' }}>
          <Link href="/" className="transition-colors hover:underline" style={{ color: '#484f58' }}>behar.io</Link>{' / '}cv
        </div>

        <style jsx global>{`
          .terminal-cursor { display: inline-block; width: 8px; height: 16px; background: #7ee787; margin-left: 2px; vertical-align: text-bottom; animation: terminal-blink 1s step-end infinite; }
